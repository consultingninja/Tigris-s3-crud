...
<h1>Update an S3 Object?!</h1>
<div class="update-wrapper">
    <div class="update-content">
        <ol>
            {#each data?.users as user,index}
            <li> <input id={`job-checkbox-${index}`} checked={selectedUser?.email === user.email} type="checkbox" value={JSON.stringify(user)} on:click={()=> selectedUser = (selectedUser?.email !== user.email)? user  : undefined}/>{`User Id: ${user.id}   Firstname: ${user.firstname}, Lastname: ${user.lastname}, Email: ${user.email}`}</li>
            {/each}
        </ol>
        {#if selectedUser}
        <form method="post" action="?/update">
            ...
            <input id="origEmail" name="origEmail" type="hidden" value={selectedUser.email} />
